        ...
        <div className="rounded-2xl border border-gold-light bg-white p-8 md:p-12 shadow-sm">
          ...
          <div className={`grid grid-cols-2 gap-6 mb-10 ${isRTL ? 'direction-rtl' : ''}`}>
            <div className={`text-center border-r border-gold-light ${isRTL ? 'border-r-0 border-l' : ''}`}>
              ...
              <p className="font-serif text-4xl text-savoia-text">{jariya}</p>
              <p className="font-sans text-xs text-savoia-muted mt-1">المعجّل</p>
            </div>
            <div className="text-center">
              <p className={`font-sans text-xs tracking-widest text-gold uppercase mb-2 ${isRTL ? 'font-arabic' : ''}`}>
                {t('template.contrat.mouwakhkhar')}
              </p>
              <p className="font-serif text-4xl text-savoia-text">{mouwakhkhar}</p>
              <p className="font-sans text-xs text-savoia-muted mt-1">المؤخّر</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className={`flex items-start gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <div>
                ...
                <p className={`font-sans text-sm text-savoia-muted ${isRTL ? 'font-arabic text-right' : ''}`}>
            ...
            <div className={`flex items-start gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <div>
                ...
                <p className="font-sans text-sm text-savoia-muted">
                  {witnesses.join(' · ')}
                </p>
              </div>
            </div>
          ...
          <div className="mt-10 pt-8 border-t border-gold-light text-center">
            <p className="font-arabic text-xl text-savoia-muted leading-loose">
              وَمِنْ آيَاتِهِ أَنْ خَلَقَ لَكُم مِّنْ أَنفُسِكُمْ أَزْوَاجًا لِّتَسْكُنُوا إِلَيْهَا
            </p>
            <p className="font-sans text-xs text-savoia-muted/50 mt-2 tracking-wide">
